...
method testEnded*(f: CodewarsOutputFormatter, testResult: TestResult) =
  ...
  of FAILED:
    ...
    if f.testStackTrace.len > 0:
      echo("\n<ERROR::>Test Error")
      if errs.len > 0:
        echo("\n<LOG:ESC:Error>" & escapeLF(failureMsg & "\n" & errs))
      else:
        echo("\n<LOG:ESC:Error>" & escapeLF(failureMsg))
